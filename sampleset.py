...
class SampleSet(object):
    ...
    def from_directory(self, directory, max_n_samples=-1, recursive=False, with_imag=True):
        ...
        if directory:    
            if not recursive:
                gen = enumerate([os.path.join(directory, f) for f in os.listdir(directory) if f.split(".")[-1] in ("mp3", "wav")])
            else:
        ...
        for _, file in gen:
            if not file.split(".")[-1] in ("mp3", "wav", "aif", "aiff"):
                ...
                continue
            i = self.N
            if max_n_samples > 0 and i >= max_n_samples:
                break
